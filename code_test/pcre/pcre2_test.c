...
int main(int argc, char *argv[]) {
	...
	aStrRegex = pattern;
#ifdef USE_JIT
	printf("Regex to use (with JIT): %s\n", aStrRegex);
#else
...
#ifdef USE_JIT
	pcreRet = pcre2_jit_compile(reCompiled, PCRE2_JIT_COMPLETE);
	if (pcreRet) {
		PCRE2_UCHAR buffer[256];
		pcre2_get_error_message(pcreRet, buffer, sizeof buffer);
		printf("ERROR: Could not jit compile '%s': %s\n", aStrRegex, buffer);
	}
	...
	for (iter = 0; iter < loop_count; iter++) {
...
#ifdef USE_JIT
		pcreExecRet = pcre2_jit_match(reCompiled,
#else
		pcreExecRet = pcre2_match(reCompiled,
#endif
						page, 
		...
		if(pcreExecRet < 0) { // Something bad happened..
			PCRE2_UCHAR buffer[256];
			pcre2_get_error_message(pcreExecRet, buffer, sizeof buffer);
			printf("ERROR: Could not match '%s': %s\n", aStrRegex, buffer);
			switch(pcreExecRet) {
...
//			case PCRE2_ERROR_UNKNOWN_NODE : printf("Something kooky in the compiled re\n");      break;
	...
	for(j=0; j<pcreExecRet; j++) {
		PCRE2_SPTR substring_start = page + ovector[2*j];
