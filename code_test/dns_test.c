...
static int
do_query(int fd, const char *name, struct in_addr *server, int port)
{
	size_t len;
	ssize_t rlen;
	char buf[128];
...
main(int argc, char **argv)
{
	...
	printf("Permanent source port is %d\n", local_port);

	while (true) {
		fd = do_query(fd, name, &server_addr, local_port);
		for (i = 1; i < queries_per_loop; i++) {
			sleep(sleep_time);
			close(do_query(-1, name, &server_addr, 0));
		}
