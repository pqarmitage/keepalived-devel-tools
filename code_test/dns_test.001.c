...
do_query(int fd, const char *name, struct in_addr *server)
{
	...
	if (fd == -1 && (fd = socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC /* | SOCK_NONBLOCK */, IPPROTO_UDP)) == -1) {
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, ":n:d:s:c:i:H")) != -1) {
		switch(opt) {
		case 'n':
			name = optarg;
			break;
		...
		case 'i':
			...
			break;
		case 'H':
			printf ("-n query_name -s DNS server -d sleep time -c queries_per_loop -i iterations (0 = forever) -H this\n");
			exit(0);
			break;
		case '?':
	...
	while (true) {
		fd = do_query(fd, name, &server_addr);
